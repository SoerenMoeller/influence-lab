...
def cache_points(problem_data: ProblemData) -> PointsCache:
    boundaries_ = {
        a: boundaries(problem_data, a) for a in problem_data.scheme.variables
    }
    ...
    for a in problem_data.scheme.variables:
        for i in range(len(boundaries_[a]) - 1):
            dist_tp(
                problem_data, points_cache, a, boundaries_[a][i], boundaries_[a][i + 1]
            )
    ...
    points_cache.sizes = {
        a: size(problem_data, points_cache, a) for a in problem_data.scheme.variables
    }
...
def boundaries(problem_data: ProblemData, variable: str) -> list[float]:
    ...
    statementsFrom = [
        st for var in scheme.order[variable] for st in scheme.statements[variable, var]
    ]
...
def dist_tp2(
    ...
    for i in range(len(bounds) - 1):
        result += 2 + dist_tp(
            problem_data, points_cache, var2, bounds[i], bounds[i + 1]
        )
...
def dom_inverse(
    scheme: Scheme, var1: str, var2: str, x: float, y: float
) -> list[Interval]:
...
def dist_poi(
    scheme: Scheme, points_cache: PointsCache, var: str, x: float, y: float
) -> int:
...
def dist_poi2(
    scheme: Scheme, points_cache: PointsCache, var1: str, var2: str, x: float, y: float
) -> int:
...
def dist(
    problem_data: ProblemData, points_cache: PointsCache, var: str, x: float, y: float
) -> int:
    return dist_tp(problem_data, points_cache, var, x, y) + dist_poi(
        problem_data.scheme, points_cache, var, x, y
    )


def original_point(
    problem_data: ProblemData, points_cache: PointsCache, var: str, x: float
) -> int:
