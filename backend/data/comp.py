...
def get_problem(
    size_domain: int, precision_max: float, precision_min: float, chain_length: int
) -> ProblemData:
    ...
    for i in range(chain_length):
        ...
        statements.extend(
            ets.collect_statements(parameters, fn, pts, f"x{i}", f"x{i + 1}")
        )
    ...
    for i in range(2, chain_length):
        ...
        parameters.range_offset = (
            precision_min
            + (precision_max - precision_min) * (chain_length - i) / chain_length
        )
